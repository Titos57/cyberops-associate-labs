# Cisco CyberOps Associate - Lab Portfolio
 
## About
Hands-on labs and research exercises completed as part of the
Cisco CyberOps Associate certification course.
Goal: Build foundational skills for a Cybersecurity Job.
 
**Course Provider:** Cisco Networking Academy
**Status:** In Progress
 
---
 
## Documentation Tiers
 
Labs in this repository are documented at one of three tiers, chosen based on the security relevance and depth of each lab:
 
- **Tier 1** — Full documentation (showcase piece): Overview, Background, Environment & Tools, full narrated Procedure Summary, Key Findings, Security Implications, References.
- **Tier 2** — Medium documentation: Overview, Background, Environment & Tools, condensed Procedure Summary, Key Findings, References.
- **Tier 3** — Brief documentation: Overview, Environment & Tools, one-paragraph Procedure Summary, References.
---
 
## Labs Completed
 
| Module | Lab | Tier | Topics Covered |
|--------|-----|------|-----------------|
| 1.0| [RFID Credit Card Skimming Analysis](./Lab-exercises/Lab%201.0%20--%20RFID%20Credit%20Card%20Skimming%20Attack%20Analysis.md) | 2 | RFID vulnerabilities, hardware security, attack vectors |
| 1.1 | [Stuxnet Attack Analysis](./Lab-exercises/Lab%201.1%20--%20Stuxnet%20Attack%20analysis.md) | 2 | Nation-state attacks, ICS/SCADA security, zero-day exploits |
| 1.2 | [IoT Smart City Infrastructure: Attack Analysis & Mitigation](./Lab-exercises/Lab%201.2%20--%20IoT%20Smart%20City%20Infrastructure%3A%20Attack%20Analysis%20%26%20Mitigation.md) | 2 | IoT vulnerabilities, smart city security, public infrastructure |
| 1.3.4 | [Threat Actor Profiling — Visualizing the Black Hats](./Lab-exercises/Lab%201.3.4%20--%20Threat%20Actor%20Profiling%3A%20Visualizing%20the%20Black%20Hats.md) | 1 | Threat actor archetypes, nation-state attacks, ransomware, insider threats |
| 2.2.5 | [Becoming a Defender — Certifications & Career Research](./Lab-exercises/Lab%202.2.5%20--%20Becoming%20a%20Defender%3A%20Certifications%20%26%20Career%20Research.md) | 1 | Cybersecurity certifications, job market research, career pathways |
| 3.0.3 | [Identify Running Processes: TCPView Analysis](./Lab-exercises/Lab%203.0.3%20--%20Identify%20Running%20Processes%3A%20TCPView%20Analysis.md) | 3 | TCPView, process monitoring, network analysis |
| 3.2.11 | [Processes Threads Handles and Windows Registry](./Lab-exercises/Lab%203.2.11%20--%20Processes%20Threads%20Handles%20and%20Windows%20Registry.md) | 1 | Windows Registry, Virus Total, Threads |
| 3.3.10 | [Create User Accounts](./Lab-exercises/Lab%203.3.10%20--%20Create%20User%20Accounts.md) | 1 | Windows user account management, Command Prompt administration |
| 3.3.11 | [Using Windows PowerShell](./Lab-exercises/Lab%203.3.11%20--%20Using%20Windows%20PowerShell.md) | 2 | Windows PowerShell, Task Manager |
| 4.2.6 | [Working with Text Files in the CLI](./Lab-exercises/Lab%204.2.6%20--%20Working%20with%20Text%20Files%20in%20the%20CLI.md) | 2 | Linux CLI, text editors, configuration files, nginx, bash |
| 4.2.7 | [Getting Familiar with the Linux Shell](.Lab-exercises/Lab%204.2.7%20--%20Getting-Familiar-with-the-Linux-Shell.md) | 2 | Linux CLI, shell navigation, file management, output redirection, hidden files, forensic awareness |
| 4.3.4 | [Linux Servers](./Lab-exercises/Lab%204.3.4%20--%20Linux-Servers.md) | 1 | Linux processes, ps, netstat, Telnet, banner grabbing, service fingerprinting, privilege separation |
| 5.1.5 | [Tracing a Route](./Lab-exercises/Lab%205.1.5%20--%20Tracing%20a%20Route.md) | 2 | Traceroute, path analysis, network diagnostics |
| 5.3.7 | [Introduction to Wireshark](./Lab-exercises/Lab%205.3.7%20--%20Introduction%20to%20Wireshark.md) | 1 | Wireshark, ping, Packet encapsulation, ICMP, MAC/IP addresses |
| 7.2.8 | [Verify IPv4 and IPv6 Addressing](./Lab-exercises/Lab%207.2.8%20--%20Verify%20IPv4%20and%20IPv6%20Addressing.md) | 3 | Packet Tracer, ipconfig /all, Traceroute, dual-stack IPv4 Ipv6 |
| 8.2.8 | [Using Wireshark to Examine Ethernet Frames](./Lab-exercises/Lab%208.2.8%20--%20Using%20Wireshark%20to%20Examine%20Ethernet%20Frames.md) | 1 | Ethernet II frame structure, Wireshark packet capture, ARP, Layer 2 vs Layer 3 addressing |
| 9.2.6 | [Using Wireshark to Observe the TCP 3-Way Handshake](./Lab-exercises/Lab%209.2.6%20--%20Using%20Wireshark%20to%20Observe%20the%20TCP%203-Way%20Handshake.md) | 2 | TCP Handshake, Tcpdump, Wireshark, Packet Analysis |
  
---
 
## Skills Demonstrated
 
- Threat actor analysis and profiling (nation-state, cybercriminal, insider)
- Vulnerability research and documentation
- Attack vector analysis (RFID, ICS/SCADA)
- Industrial Control System (ICS) security concepts
- Zero-day vulnerability analysis
- Critical infrastructure threat assessment
- Professional security report writing
- MITRE ATT&CK framework awareness
- IoT security analysis
- Smart city infrastructure vulnerability assessment
- Public sector cybersecurity risk evaluation
- Ransomware attack analysis
- Insider threat detection and mitigation
- Corporate espionage case analysis
- Cybersecurity certification landscape research
- Job market analysis
- Career pathway planning in cybersecurity
- Network packet capture and analysis (Wireshark)
- Layer 2/Layer 3 addressing and traffic path analysis
- Windows user account and system administration
- Network diagnostics (traceroute, path analysis)

---

## Contact

**Name:** Christos Panopoulos

**LinkedIn:** [https://www.linkedin.com/in/cpanopoulos/](https://www.linkedin.com/in/cpanopoulos/).  

**Email:** christospanopoulos9@gmail.com 

**GitHub:** [github.com/Titos57](https://github.com/Titos57)

**HackTheBox** [https://profile.hackthebox.com/profile/019d0ced-6ed8-721f-b8fe-bf94f048faaa](https://profile.hackthebox.com/profile/019d0ced-6ed8-721f-b8fe-bf94f048faaa)

**TryHackMe** [https://tryhackme.com/p/Titos57](https://tryhackme.com/p/Titos57)
